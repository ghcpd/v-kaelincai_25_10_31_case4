"""Test harness validating stability of optimized implementation.
Generates: results_optimized.json, log_optimized.txt, time_optimized.txt
"""
...
for case in test_cases:
    ...
    for i in range(N_RUNS):
        ...
        try:
            res = optimized_code.process_case(case)
            reported = res["reported_sum"]
            sums.append(reported)
            durations.append(res["duration_sec"])
            LOG_LINES.append(f"CASE {case_id} RUN {i} => sum={reported} duration={res['duration_sec']:.5f}")
...
with open(os.path.join(OUT_DIR, 'results_optimized.json'), 'w', encoding='utf-8') as f:
    json.dump({"runs": RESULTS, "total_cases": len(RESULTS)}, f, indent=2)
with open(os.path.join(OUT_DIR, 'log_optimized.txt'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(LOG_LINES))
with open(os.path.join(OUT_DIR, 'time_optimized.txt'), 'w', encoding='utf-8') as f:
    f.write('\n'.join(TIMING_LINES))

print("Optimized stable test execution complete. See results_optimized.json and log_optimized.txt")
